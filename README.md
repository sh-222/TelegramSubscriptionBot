...
2.  **Защита группы**: Добавьте бота в свою группу как администратора.
...
*   `/del_channel <id>` — Удалить канал из базы данных (отключить проверку).

**Отключение проверок**:
Чтобы бот перестал требовать подписку, выполните одно из действий:
1.  Удалите канал из базы командой `/del_channel <id>`.
2.  Удалите бота из администраторов группы.
3.  Удалите бота из администраторов канала (он автоматически удалит канал из базы).
...
*   `/del_channel <id>` — Remove a channel from the database.

**Disabling Checks**:
To stop subscription enforcement, do one of the following:
1.  Remove the channel using `/del_channel <id>`.
2.  Remove the bot from the group administrators.
3.  Remove the bot from the channel administrators (auto-removes from DB).
